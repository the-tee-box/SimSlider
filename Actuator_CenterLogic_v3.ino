...
// Center position timing: half of full travel (16000 / 2 = 8000ms from either endpoint)
const unsigned long MOVE_TIME_TO_CENTER = 8000;   // Time to reach center from LH or RH (ms)

// State tracking
enum Position { POS_UNKNOWN, POS_RH, POS_LH, POS_CENTER, POS_MOVING };
...
void setup() {
  ...
  Serial.println("Commands: RH, LH, CENTER, STOP, STATUS, HOME");
  Serial.flush();
  delay(100);
  reportStatus();
}
...
void moveTo(Position target) {
  ...
  if (currentPosition == target) {
    ...
    if (target == POS_RH)     Serial.println("RH");
    else if (target == POS_LH)    Serial.println("LH");
    else if (target == POS_CENTER) Serial.println("CENTER");
    Serial.flush();
    return;
  }
  ...
  if (target == POS_CENTER) {
    if (currentPosition == POS_LH || currentPosition == POS_RH) {
      // Known endpoint → center is exactly half travel
      moveExpectedTime = MOVE_TIME_TO_CENTER;
    } else {
      // Unknown position → use full travel time to be safe (limit switch protects us)
      moveExpectedTime = max(MOVE_TIME_LH_TO_RH, MOVE_TIME_RH_TO_LH) + MOVE_BUFFER;
    }
  }
  else if (currentPosition == POS_UNKNOWN || currentPosition == POS_CENTER) {
    // From center or unknown, use full travel time
    moveExpectedTime = max(MOVE_TIME_LH_TO_RH, MOVE_TIME_RH_TO_LH) + MOVE_BUFFER;
  }
  ...
  Position priorPosition = currentPosition;  // Save before overwriting
  ...
  else if (target == POS_CENTER) {
    Serial.println("Moving to CENTER position...");
    Serial.print("Expected time: ");
    Serial.print(moveExpectedTime / 1000.0);
    Serial.println(" seconds");
    Serial.flush();
    // From LH → extend toward RH to reach midpoint
    // From RH → retract toward LH to reach midpoint
    // From unknown → extend (times out at midpoint; limit switch prevents over-travel)
    if (priorPosition == POS_RH) {
      moveLeft();
    } else {
      moveRight();
    }
...
void checkMovement() {
  ...
  if (elapsed >= moveExpectedTime) {
    ...
    if (targetPosition == POS_RH)     Serial.println("RH position");
    else if (targetPosition == POS_LH)    Serial.println("LH position");
    else if (targetPosition == POS_CENTER) Serial.println("CENTER position");
  ...
  if (elapsed >= 5000 && (elapsed - lastProgressUpdate >= 5000)) {
    ...
    Serial.print("s → ");
    if (targetPosition == POS_RH)      Serial.println("RH");
    else if (targetPosition == POS_LH) Serial.println("LH");
...
void updateLEDs() {
  if (eStopActive) {
    ...
    if (currentPosition == POS_CENTER) {
      // Both LEDs on to indicate center position
      digitalWrite(LED_LH, HIGH);
...
void reportStatus() {
  ...
  if (eStopActive) {
    ...
    switch (currentPosition) {
      ...
      case POS_CENTER:  Serial.println("CENTER");  break;
